...
const About = () => {
  ...
  const stats = [
    ...
    { value: "1", label: "Patents" },
    { value: "40%", label: "Membership Growth" },
    { value: "Ph.D.", label: "Ongoing" }
  ];
  ...
  return (
    <section id="about" className={`bg-black ${isMobile ? 'py-12 overflow-hidden' : 'py-16 sm:py-20'}`}>
      <div className={`container mx-auto ${isMobile ? 'px-3 max-w-full overflow-hidden' : 'px-4 sm:px-6'}`}>
        ...
        >
          <h2 className={`font-bold text-teal-500 font-['Fredoka'] ${isMobile ? 'text-3xl' : 'text-4xl sm:text-5xl md:text-6xl'}`}>
            About Me
          </h2>
        </motion.div>

        {/* Two Column Layout - Different for mobile and desktop */}
        <div className={`${isMobile ? 'flex flex-col gap-6 mb-10' : 'grid lg:grid-cols-2 gap-8 md:gap-12 max-w-6xl mx-auto mb-12 sm:mb-16 md:mb-20'}`}>
          ...
          >
            <p className={`text-gray-300 leading-relaxed mb-3 ${isMobile ? 'text-sm' : 'text-base sm:text-lg md:text-xl'}`}>
              I'm an innovative researcher with expertise in Beyond 5G (B5G) mobile networks, 
              Software-Defined Networking (SDN), AI-driven handover decision-making algorithms, 
            ...
            <p className={`text-gray-300 leading-relaxed ${isMobile ? 'text-sm' : 'text-base sm:text-lg md:text-xl'}`}>
              Currently pursuing my Ph.D. at IIIT Naya Raipur, I'm passionate about pushing the boundaries 
              of next-generation wireless communication through AI/ML-driven solutions.
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            className={`grid grid-cols-2 gap-3 content-start ${isMobile ? 'gap-2' : 'gap-3 sm:gap-4 md:gap-6'}`}
          >
            {stats.map((stat, index) => (
              <div key={index} className={`text-center bg-gray-900/50 rounded-lg border border-gray-800 hover:border-gray-700 transition-all ${isMobile ? 'p-3' : 'p-3 sm:p-4 md:p-6'}`}>
                <div className={`font-bold text-white ${isMobile ? 'text-2xl' : 'text-2xl sm:text-3xl md:text-4xl lg:text-5xl'}`}>{stat.value}</div>
                <div className={`text-gray-400 ${isMobile ? 'text-xs mt-1' : 'text-xs sm:text-sm md:text-base mt-1 sm:mt-2'}`}>{stat.label}</div>
              </div>
            ))}
          </motion.div>
        </div>
        ...
        >
          <h3 className={`font-bold text-teal-500 text-center font-['Fredoka'] ${isMobile ? 'text-2xl mb-6' : 'text-3xl sm:text-4xl md:text-5xl mb-8 sm:mb-10 md:mb-12'}`}>
